    ...
    >
      <div
        className={`relative rounded-xl border transition-all duration-300 overflow-hidden group
          ${focused
            ? "border-primary/60 bg-card shadow-[0_0_0_3px_color-mix(in_srgb,var(--primary)_18%,transparent)]"
            : hasError
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 pt-3 pb-1">
          ...
          <span className={`text-caption font-semibold uppercase tracking-[0.18em] transition-colors duration-200 ${focused ? "text-primary" : "text-muted-foreground/50"}`}>
            {label}
          </span>
        </div>
      ...
      <AnimatePresence>
        {hasError && (
          <motion.p
            className="flex items-center gap-1 mt-1.5 px-1 text-caption text-destructive"
...
const SubmitButton: React.FC<{ status: Status; lang: string }> = ({ status, lang }) => {
  ...
  const idleBackground = "var(--primary)";
  ...
  return (
    <motion.button
      ...
      className={`group relative w-full flex items-center justify-center py-3.5 rounded-xl font-semibold text-small text-primary-foreground transition-all duration-300 overflow-hidden
        ${status === "success" ? "bg-gradient-to-r " + colors[status] : ""}
        disabled:opacity-80 disabled:cursor-not-allowed
        before:absolute before:inset-0 before:bg-foreground/0 before:transition-colors before:duration-300 hover:before:bg-foreground/[0.08]`}
...
const ContactForm: React.FC = () => {
  ...
  const triggerConfetti = useCallback(() => {
    if (confettiLib) {
      confettiLib({
        ...
        spread: 90,
        origin: { y: 0.6 },
        // Le canvas confetti exige des couleurs littérales : on lit l'accent
        // effectif du thème au moment du tir plutôt que de le figer.
        colors: [
  ...
  return (
    <div className="relative w-full">
      ...
      <BorderBeam size={280} duration={8} colorFrom="var(--primary)" colorTo="var(--ring)" />
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-3 pt-1">
        ...
        <Field icon={MessageSquare} label={language === "fr" ? "Message" : "Message"} error={formik.errors.message} touched={formik.touched.message} index={2}>
          <textarea
            name="message"
            placeholder={language === "fr" ? "Ex. : besoin, fonctionnalités, stack actuelle, délai…" : "E.g. need, features, current stack, timeline…"}
      ...
      <AnimatePresence>
        {messageSent && (
          <motion.div
            key="toast"
            className="mt-4 flex items-center gap-3 rounded-xl border border-success/25 bg-success/10 px-4 py-3 text-small text-success"
