...
export const FloatingDock = ({
  ...
  items: {
    ...
    href: string;
    /** Ajout : le bouton de thème n'est pas une navigation. */
    onClick?: (event: React.MouseEvent<HTMLAnchorElement>) => void;
    /** Ajout : marque l'élément correspondant à la section visible. */
    active?: boolean;
  }[];
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-3 rounded-2xl border border-border bg-card/90 px-4 pb-3 shadow-lg backdrop-blur-xl md:flex",
        className,
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md bg-primary px-2 py-1 text-xs whitespace-pre text-primary-foreground shadow-md"
            >
              {title}
            </motion.div>
          )}
